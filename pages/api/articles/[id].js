...
const getArticle = async (req, res) => {
    try {
        const {id} = req.query
        const [result] = await pool.query("SELECT * FROM article WHERE articleid = ?", [id]);
...
const updateArticle = async (req, res) => {
    ...
    try {
        await pool.query("UPDATE article SET articletitle = ?, description = ?, price = ? WHERE articleid = ?", [articletitle, description, price, id]);
        return res.status(204).json();
    } catch (error) {
        return res.status(500).json({message: error.message});
    }
